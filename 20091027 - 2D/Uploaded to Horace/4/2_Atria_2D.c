...
int main(int argc, char **argv)
{
   ...
   float global_V[X+314+1][Y+1]; // added +l to x component to extend sheet as above, didn't need to for y axis as no extension in this direction
   float new_V[X+314+1][Y+1];
   float dvdt[X+314+1][Y+1];
   int geometry[X+314+1][Y+1];
...
// ** Initialise all variables at all co-ordinated ** //
   for (x=0; x<=X; x++) {
     ...
     for (y=0; y<=Y; y++) {
       ...
       V[x][y]=-73.8; /* use this or similar as a boundary condition for the bottom of the pulmonary vein sheet 
						need to make equal the sides of the sheet so as to produce a cylinder */
	   new_V[x][y]=-73.8;
       Pa[x][y]=0.000028;
       Pi[x][y]=0.933953;
       n[x][y]=0.00925;
...
/*//PV Coordinates
       //  if((y>=75 && y<= 125) && (x>=75 && x<=125)){  commented out
		 { /* thinking we copy this to produce a new sheet with boundary conditions at base to match cardiac tissue but also with equal
		   sides to as to make a cylinder 
                   int pv_radius = sqrt(pow(x-100,2) + pow(y-100,2)); //commented out 
...
// ** Define the differential approximation ** //
	 ...
	 switch (geometry[x][y]) {
       ...
       case 5: dvdt[x][y]=0.25*D*(dvdx2+dvdy2)-I_tot_PV(x, y); break; //D is the diffusion coefficient, notice smaller for PV.  now halved to 0.25
...
/**** new sheet for pulmonary vein in 3D  ****/

for (x=600;x<=600+314;x++)
  {
       for (y=0; y<=Y; y++) {
...
{    // boundary condition on the x axis forcing a cylinder shape where both ends are of equal V
 //V[600][y] = V[600+314][y]; //NEED TO GET THIS RIGHT
}

for (x=600; x<=600+314; x++) //for the bottom of the sheet to match the pulmonary vein area of the main sheet
{
 V[x][0] = V[75][75];
}
