...
//PV Coordinates
       //  if((y>=75 && y<= 125) && (x>=75 && x<=125)){
                    pv_radius = sqrt(pow(x-100,2) + pow(y-100,2));
                         if(pv_radius>=0 && pv_radius<=50){ //changed from 25 to 50
                    geometry[x][y]=5;
                    }
		 }
		}
...
// ** Define the differential approximation ** //
	 ...
	 switch (geometry[x][y]) {
       case 1: dvdt[x][y]=D*(dvdx2+dvdy2)-I_tot_PM(x, y); break; //D is the diffusion coefficient, notice smaller in case 5
       ...
       case 5: dvdt[x][y]=0.5*D*(dvdx2+dvdy2)-I_tot_PV(x, y); break;
	...
	for (x = BLOCK_LOW(rank, size, X+1); x <= BLOCK_HIGH(rank, size, X+1); x++) 
{
    ...
    { 
     ...
     if (cnt % 1000 == 0) {    //changed, 1 zero removed 
