...
float I_tot_CS (int x, int y, int z)
{
   ...
   Ito=0.31*50.02*r1[x][y][z]*(0.590*pow(s1[x][y][z],3)+0.410*pow(s2[x][y][z],3))*(0.600*pow(s3[x][y][z],6)+0.4)*(V[x][y][z]-Ek); 
   Isus = 0.4*(V[x][y][z]+85)+0.2*(V[x][y][z]-5);
   Ito+=Isus;
   ...
   s1m=1./(1+exp((V[x][y][z]+24.8)/5.6));
   ts1=0.0496/(1+exp((V[x][y][z]+32.8)/0.1))+0.0158; // t1_r-t1_d / (Exp) + t1_d
   s1[x][y][z]+=HT*(s1m-s1[x][y][z])/ts1;

   s2m= 1./(1+exp((V[x][y][z]+24.8)/5.6)); 
   ts2=2.9536/(1+exp((V[x][y][z]+32.8)/0.1))+0.1245; // t2_r+t2_d / (Exp) + t2_d
...
//   m[x][y][z]+=HT*(Am*(1-m[x][y][z])-Bm*m[x][y][z]);
   ...
   dlm=1./(1+exp(-(V[x][y][z]-0.5)/5.7));
   dL[x][y][z]+=HT*(dlm-dL[x][y][z])/tdl;


  flm=1./(1+exp((V[x][y][z]+17.9)/5.2));
  tfl1=(0.0335/(1+exp((V[x][y][z]+18)/4.0)))+0.0117;
  tfl2=(2.2989/(1+exp((V[x][y][z]+18)/4.0)))+0.0478;
...
*/
  ICaL=0.6*29.7*(dL[x][y][z]*((0.80*fL1[x][y][z])+(0.2*fL2[x][y][z])))*(V[x][y][z]-58.3);   
...
/*
  ...
  Itr=(Caup[x][y][z]-Carel[x][y][z])*2.*Faraday*Vup/0.01;
   ...
   Carel[x][y][z]+=HT*((Itr-Irel)/(2.*Faraday*Vrel)-31.0*(480*Carel[x][y][z]*(1-faCalse)-400*faCalse));   
