...
//   float dfca;
   ...
   Ito=0.54*50.02*r1*(0.590*pow(s1,3)+0.410*pow(s2,3))*(0.600*pow(s3,6)+0.4)*(V-Ek);     
   Isus = 0.3*(V+85);
   ...
   s1m=1./(1+exp((V+28.2)/5.3));
   ts1=0.1199/(1+exp((V+32.8)/0.1))+0.0157; // t1_r-t1_d / (Exp) + t1_d
   s1+=HT*(s1m-s1)/ts1;

   s2m= 1./(1+exp((V+28.2)/5.3)); 
   ts2=3.1616/(1+exp((V+32.8)/0.1))+0.1103; // t2_r+t2_d / (Exp) + t2_d
   s2+=HT*(s2m-s2)/ts2;
   ...
   dlm=1./(1+exp(-(V-7.9)/6.3));
   dL+=HT*(dlm-dL)/tdl;

  flm=1./(1+exp((V+20.4)/6.3));
  tfl1=(0.0291/(1+exp((V+18)/4.0)))+0.0132;
  fL1+=HT*(flm-fL1)/tfl1;
/*
  dfca=1.0/(1+Cai/0.00015);
  fca+=HT*(dfca-fca)/0.02;
*/
  ICaL=0.6*16*dL*fL1*(V-61.4); 
