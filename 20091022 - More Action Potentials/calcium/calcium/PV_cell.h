...
float I_tot_PV ()
{
   ...
   Ito=0.45*0.515*50.02*r1*(0.8*pow(s1,3)+0.2*pow(s2,3))*(V-Ek);     // CT - 0.2, PM - 0.35
...
//SAP
//INaCa = 1.5*(0.02*(pow(Nai,3)*7.5*exp(0.35*V/RTONF)-pow(140,3)*Cai*exp(V*(0.45-1)/RTONF))/(1+0.0003*(Cai*pow(140,3)+2.5*pow(Nai,3)))); //1.5
...
//   m+=HT*(Am*(1-m)-Bm*m);
  ...
  dfca=1.0/(1+Cai/0.00001);  //0.00025
  fca+=HT*(dfca-fca)/0.04;
...
//   ICaL=18*(dL*((0.80*fL1)+(0.2*fL2)))*fca*(V-61.4);   
   ...
   Iup=3*2800.0*(((Cai/0.0003)-(0.4*0.4*Caup/0.5))/(((Cai+0.0003)/0.0003)+(0.4*(Caup+0.5)/0.5)));

   Irel=4*200000.0*pow(F2/(F2+0.25),2)*(Carel-Cai);    
