...
#define BCL 10		// seconds
#define beats 1		// change to 10, originally 5

float IKf,IKs,IK,Ik1,Ito,IbNa,Ip,INaCa,IbCa,INa, ICap,ICaL,ICaT,IbK, Itot,Iup,Itr,Irel,Isus;  //Currents
...
float V=-80.0;	//initial conditions for all vars
float Pa=0.00016;
float Pi=0.76898;
float n=0.02032;
float r1=0.00006;
...
float fT=0.30752;
float Nai=8.4;
float Caup=0.730866;/// lower these, one at a time, to try and force action potentials WITHOUT a stimulus
...
const float Cm=0.05;		//membrance capacitance
...
//   printf ("APD = %8.2lf, dVdtmax = %8.2lf\n", 1000*apd[beats], 0.001*dvdtmax[beats]);
   printf ("APD = %8.2lf, dVdtmax = %8.2lf\n", 1000*apd[beats], 0.001*dvdtmax[beats]);
   printf ("APD-1 = %8.2lf, dVdtmax = %8.2lf\n", 1000*apd[beats-1], 0.001*dvdtmax[beats]);
