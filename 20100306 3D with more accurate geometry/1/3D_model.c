...
//this is the line in the geometry-creating C file that relates to CT cells: if((y>=325 && y<= 350) && (x>=25 && x<=275))
   for (x=25; x<=275; x++)
	 for (y=325; y<=350; y++)
	   for (z=0; z<=10; z++)
		 if (h[x][y][z] > 0) {              
...
// ** Begin voltage calculation ** //
 ...
 //this next bit does the repeating stimulus.  leave it out for this one
   /*if (t>=(girishcounter*0.04) && t<((girishcounter*0.04)+HT)) // 0.04 = 400ms
	   {
	   for (x=0; x<=10; x++)
	   for (y=0; y<=Y; y++)
	   for (z=0; z<=10; z++)
