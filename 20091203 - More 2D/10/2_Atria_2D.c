...
// ** Set the stimulus sites ** //
   ...
   for (x=0; x<=3; x++)
	 for (y=0; y<=Y+lucy; y++) // added +lucy here to get it stimulating all over y, not sure if necessary
       V[x][y] = 20.0;
...
//PM Coordinates
		 if ((y>235 && y<= 300) && ((x >= 20 && x < 35) || (x >= 50 && x < 65) || (x >= 80 && x < 95) ||
                                    (x >= 110 && x < 125) || (x >= 140 && x <155 ) || (x >= 170 && x < 185))){
                                    geometry[x][y]=1; 
                                    }
//CT Coordinates
         if((y>=215 && y<= 235) && (x>=15 && x<=185)){
                    geometry[x][y]=2;
                    }
//BB Coordinates
         if((y>=165 && y<= 235) && (x>=0 && x<15)){
                    geometry[x][y]=3;}
//CS Coordinates
         if((y>=165 && y<= 235) && (x>=185 && x<X)){
                    geometry[x][y]=4;}
...
//PV Coordinates
   ...
   while (t < 100.0) { 
     ...
     for (x = BLOCK_LOW(rank, size, X+1); x <= BLOCK_HIGH(rank, size, X+1); x++) {
	   ...
	   if (y != Y && y != Y+1 && ((x-65)*(x-65)+(y-65)*(y-65))>=pv_radius*pv_radius) // only do this if you're outside the hole where PV connects
...
// ** Zero-flux conditions at the atrial septum ** //
	   if (x >= 20 && x <= 180) {
	     new_V[x][200]=new_V[x][199];
	     new_V[x][201]=new_V[x][202];
	   }
...
{    //for the bottom of the sheet to match the pulmonary vein area of the main sheet
  ...
  ymap = 65+ceil(pv_radius*cos(2*3.1415926*(y-Y)/lucy)); // PROVEN to create a circle in 20091201/Circle3
  xmap = 65+ceil(pv_radius*sin(2*3.1415926*(y-Y)/lucy));

//  new_V[xmap][ymap] = 20.0; // to create a red ring around the pulmonary vein

  ymap1 = 65+ceil((pv_radius+1)*cos(2*3.1415926*(y-Y)/lucy));
  xmap1 = 65+ceil((pv_radius+1)*sin(2*3.1415926*(y-Y)/lucy));

  new_V[0][y] = new_V[xmap][ymap]; //changed from V
  new_V[xmap1][ymap1] = (1/3.1415926)*new_V[1][y]+((1-(1/(2*3.1415926)))*new_V[xmap1][ymap1]); // ARITHMETIC TYPE
}
...
// ** Replace the old Voltage values with the new ones ** //
  for (x = BLOCK_LOW(rank, size, X+1); x <= BLOCK_HIGH(rank, size, X+1); x++) 
    for (y=0; y<=Y+lucy; y++)
	if (((x-65)*(x-65)+(y-65)*(y-65))>=pv_radius*pv_radius)
    { 
		V[x][y]=new_V[x][y];
    }
   ...
   if (t>=0.04 && t<(0.04+HT))
	 ...
	 if (rank < (size-1))
       MPI_Send(V[BLOCK_HIGH(rank, size, X+1)], Y+lucy+1, MPI_FLOAT, rank+1, 0, MPI_COMM_WORLD);
     if (rank > 0)
       MPI_Recv(V[BLOCK_HIGH((rank-1), size, X+1)], Y+lucy+1, MPI_FLOAT, rank-1, 0, MPI_COMM_WORLD, &status);
     if (rank > 0)
       MPI_Send(V[BLOCK_LOW(rank, size, X+1)], Y+lucy+1, MPI_FLOAT, rank-1, 1, MPI_COMM_WORLD);
     if (rank < (size-1))
       MPI_Recv(V[BLOCK_LOW((rank+1), size, X+1)], Y+lucy+1, MPI_FLOAT, rank+1, 1, MPI_COMM_WORLD, &status);
