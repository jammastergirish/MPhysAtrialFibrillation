...
//PV Coordinates
   ...
   while (t < 100.0) { 
     ...
     for (x = BLOCK_LOW(rank, size, X+1); x <= BLOCK_HIGH(rank, size, X+1); x++) {
	   ...
	   if (y != Y && y != Y+1 && ((x-100)*(x-100)+(y-100)*(y-100))>=pv_radius*pv_radius) // only do this if you're outside the hole where PV connects
...
{    //for the bottom of the sheet to match the pulmonary vein area of the main sheet
  ...
  ymap = 100+ceil(pv_radius*cos(2*3.1415926*(y-Y)/lucy)); // PROVEN to create a circle in 20091201/Circle3
  xmap = 100+ceil(pv_radius*sin(2*3.1415926*(y-Y)/lucy));

  new_V[xmap][ymap] = 20.0; // to create a red ring around the pulmonary vein

  ymap1 = 100+ceil((pv_radius+1)*cos(2*3.1415926*(y-Y)/lucy));
  xmap1 = 100+ceil((pv_radius+1)*sin(2*3.1415926*(y-Y)/lucy));
