...
float I_tot_CS (int x, int y)
{
   ...
   Ito=0.31*50.02*r1[x][y]*(0.590*pow(s1[x][y],3)+0.410*pow(s2[x][y],3))*(0.600*pow(s3[x][y],6)+0.4)*(V[x][y]-Ek); 
   Isus = 0.4*(V[x][y]+85)+0.2*(V[x][y]-5);
   Ito+=Isus;
   ...
   s1m=1./(1+exp((V[x][y]+24.8)/5.6));
   ts1=0.0496/(1+exp((V[x][y]+32.8)/0.1))+0.0158; // t1_r-t1_d / (Exp) + t1_d
   s1[x][y]+=HT*(s1m-s1[x][y])/ts1;

   s2m= 1./(1+exp((V[x][y]+24.8)/5.6)); 
   ts2=2.9536/(1+exp((V[x][y]+32.8)/0.1))+0.1245; // t2_r+t2_d / (Exp) + t2_d
...
//   m[x][y]+=HT*(Am*(1-m[x][y])-Bm*m[x][y]);
   ...
   dlm=1./(1+exp(-(V[x][y]-0.5)/5.7));
   dL[x][y]+=HT*(dlm-dL[x][y])/tdl;


  flm=1./(1+exp((V[x][y]+17.9)/5.2));
  tfl1=(0.0335/(1+exp((V[x][y]+18)/4.0)))+0.0117;
  tfl2=(2.2989/(1+exp((V[x][y]+18)/4.0)))+0.0478;
...
*/
  ICaL=0.6*29.7*(dL[x][y]*((0.80*fL1[x][y])+(0.2*fL2[x][y])))*(V[x][y]-58.3);   
...
/*
  ...
  Itr=(Caup[x][y]-Carel[x][y])*2.*Faraday*Vup/0.01;
   ...
   Carel[x][y]+=HT*((Itr-Irel)/(2.*Faraday*Vrel)-31.0*(480*Carel[x][y]*(1-faCalse)-400*faCalse));   
